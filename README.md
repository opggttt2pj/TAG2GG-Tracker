...
캐릭터 ID는 Tracker와 웹 프론트엔드가 공유하는 매핑으로 한글 이름과 이미지
경로로 변환됩니다. 원본 경기 행을 기반으로 대시보드, TOP 10, 최근 대전,
플레이어 프로필, 캐릭터 픽률, 태그 조합, 상대 전적을 클라이언트에서 계산합니다.

## 웹사이트 동작 방식

웹사이트는 별도의 서버 애플리케이션 없이 `ttt2_web/index.html`을 중심으로
동작하는 정적 프론트엔드입니다.
...
- Tracker 업로드 실패는 콘솔에 표시되며, 해당 경기의 재전송 큐를 별도로
  제공하지 않으므로 운영 시 Supabase 로그와 Tracker 로그를 함께 확인해야 합니다.
